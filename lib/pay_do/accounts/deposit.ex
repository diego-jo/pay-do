defmodule PayDo.Accounts.Deposit do
  alias Ecto.Multi
  alias PayDo.{Account, Repo}

  def call(%{"id" => id, "value" => value}) do
    Multi.new()
    |> Multi.run(:account, fn repo, _useless -> get_account(repo, id) end)
  ...
  defp handle_cast(:error, _useless), do: {:error, "Invalid deposit value!"}

  defp update_account({:error, _useless} = error, _repo, _account), do: error
